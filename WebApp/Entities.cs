...
namespace InvApp.Entities
{
    ...
    public class Stock
    {
        ...
        public decimal? TotalFinResult(AccountType? account, Currency? cur) 
        {
            ...
            { 
                var v = Invest.WebApp.Core.Instance.Operations
		...
		{
			var r = Invest.WebApp.Core.Instance.Operations.Where(x => x.Type == OperationType.Buy
		...
		{
			var r = Invest.WebApp.Core.Instance.Operations.Where(x => x.Type == OperationType.Sell
			                                                          && x.Stock.Ticker == Ticker
			                                                          && (type == null || x.AccountType == type)
				);
	        ...
	        foreach(var item in Items)
	        {
		        ...
		        if (item.ForCalc)
		        {
					...
					while (lots > 0)
					{
						...
						{
							var buyRate = Invest.WebApp.Core.Instance.GetCurRate(op.Currency, opBuy.Operation.DeliveryDate.Value);
							var sellRate = Invest.WebApp.Core.Instance.GetCurRate(op.Currency, op.DeliveryDate.Value);
						...
						if (!Invest.WebApp.Core.Instance.FifoResults.ContainsKey(analitic))
						{
							Invest.WebApp.Core.Instance.FifoResults.Add(analitic, result);
						}
						else
						{
							var r = Invest.WebApp.Core.Instance.FifoResults[analitic];
   ...
   //     }
    ...
    public class Operation
    {
        ...
        public decimal? Commission => Invest.WebApp.Core.SumValues(BankCommission1, BankCommission2);
    }
