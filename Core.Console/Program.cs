...
namespace Invest.Core.Console
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //
            _apiController = new ApiController(/*dataProvider, _jobManager*/ /*, config.ServiceApi*/);
        ...
        private static void InitCore(Config config)
        {
	        ...
	        var portfolios = builder.LoadPortfoliosFromJson(fileName);
	        builder.SetPortfolios(portfolios);
	        ...
	        System.Console.WriteLine($"\t- Stocks loaded ({builder.Stocks.Count})");
			...
			foreach(var b in config.Import.Brokers) 
			{
				...
				if (b.Id == "vtb")
					importInstance = new VtbBrokerReport(reportDir, builder);
				else if (b.Id == "sber")
					importInstance = new VtbBrokerReport(reportDir, builder);
				else if (b.Id == "ab")
					importInstance = new VtbBrokerReport(reportDir, builder);
